...
class AW_Lib_Model_AdminhtmlPdf_Sales_Order_Pdf_Shipment extends Mage_Sales_Model_Order_Pdf_Shipment
{
    // Column count in layout section
    const ONE_COLUMN_LAYOUT = 1;
    const TWO_COLUMN_LAYOUT = 2;

    protected $_pdfShipmentInterface = 'AW_Lib_Model_AdminhtmlPdf_Sales_Order_Pdf_ShipmentInterface';
     ...
     *
     * @return AW_Lib_Model_AdminhtmlPdf_Sales_Order_Pdf_Shipment
     */
    public function collectSectionModels()
    {
        $configData = Mage::getConfig()->getNode("aw_lib/models/pdf/shipment")->asArray();
        ...
        foreach ($configData as $moduleCode => $moduleConfig) {
            if (array_key_exists('model', $moduleConfig)) {
                ...
                if (($modelInstance instanceof $this->_pdfShipmentInterface) && $modelInstance->canDraw()) {
